...
async def _inbox_analyze_callback(
    *, device_id: str, pages: list, target_language: str, source: str
    ...
    # enough for slowapi to not blow up. The route handler moved to
    # routers.scan in Phase 3a, so we import it lazily here to avoid a
    # circular dependency (main.py imports server, server imports routers).
    class _FakeReq:
