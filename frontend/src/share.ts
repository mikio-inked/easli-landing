...
function buildAnalysisHtml(record: AnalysisRecord, lang: LanguageCode): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>easli</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  <div class="footer">Generated by easli • ${escapeHtml(record.target_language_label || record.target_language)}</div>
</body></html>`;
}
...
export async function shareAnalysisAsText(
  ...
  if (Platform.OS === 'web') {
    try {
      ...
      if (navAny?.share) {
        await navAny.share({ title: 'easli', text });
        return;
      }
      if (navAny?.clipboard?.writeText) {
        await navAny.clipboard.writeText(text);
        Alert.alert('easli', 'Copied to clipboard.');
        return;
      }
    } catch (e) {
      // fall through
    }
    Alert.alert('easli', text);
    return;
  }
  ...
  try {
    await Share.share({ message: text, title: 'easli' });
  } catch (e) {
    Alert.alert('easli', t(lang, 'share_failed'));
  }
}
...
export async function shareAnalysisAsPdf(
  ...
  try {
    ...
    if (Platform.OS === 'ios') {
      ...
      await Share.share({ url: uri, title: 'easli' });
      return;
    }
